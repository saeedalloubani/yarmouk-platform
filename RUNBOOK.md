...
>
> **D67 update (2026-06)**: per-category labels for the 4 pilot variants. Pre-D67 `lib/i18n.ts` shipped one `categoryOfficials` + one `pilotBadge` string and rendered them for ALL categories regardless of `invitation.category`. D66 smoke (SMOKE-D66-002, category=researchers) surfaced a Researcher landing page showing "Official — Pilot Reviewer". D67 added 3 new `categoryX` keys + renamed `pilotBadge` → `pilotBadgeOfficials` with 3 new sibling badge keys, plus `categoryLabel(category, t)` + `pilotBadgeLabel(category, t)` helpers in `lib/i18n.ts`. No migration, no RPC, no schema change — i18n strings + helpers + 4 consumer migrations only.
>
> **D68 update (2026-06)**: stripped "Pilot" wording from every participant-facing surface — landing eyebrow ("· Pilot Phase" suffix dropped), invited card category labels ("— Pilot Reviewer" suffix dropped from all 4), questionnaire shell badge ("Pilot Version 1 · …" — removed entirely; Y3 — version tracking is backend-only via `questionnaire_version_id`), feedback section heading + intro + map-hint all phase-agnostic. `studyTitle` adopts the formal thesis-citation phrasing ("Evaluating the 1987 agreement between Jordan and Syria regarding the utilization of Yarmouk River water"). The `pilotBadge*` i18n keys, `pilotBadgeLabel` helper, and `variantToPilotCategory` admin-preview function are retained as inert dead code (A2 deferred-cleanup carve-out) — each carries an explicit "Unused since D68; remove in a later cleanup cycle" comment. **Admin-side "Pilot" mentions intentionally kept**: `/admin` Overview eyebrow + variant card "Pilot · Version 1", `/admin/analytics/feedback` H1, `lib/repos/questionnaires.ts` `variantLabel` map, `lib/repos/feedback.ts` `getPilotFeedback` symbol. Different concern — internal study-phase marker for Sura/Saeed, not a participant-facing surface. No migration, no RPC, no schema change, no email-template edit — pure i18n + 2 JSX badge removals + 2 prop drops.
>
> **D69 update (2026-06)**: deferred cleanup batch. (a) D68 A2 dead code removed — `pilotBadgeX` i18n keys, `pilotBadgeLabel` helper, `variantToPilotCategory` admin-preview function (zero live consumers — every reference was the symbol's own definition or annotation). (b) `collection_mode` added to `invitations_redacted` view (migration `20260602130000`) — readonly admins now see it; closes a latent runtime-undefined bug in `lib/repos/invitations.ts` where the mapper was reading `r.collection_mode` from a view row that didn't project it (watch-out #5 NOT-NULL-recovery cast was hiding the read). View grows 22 → 23 columns; forward-only. (c) `TASK_STATE.md` top-block refresh covering D63 → D69 closures (minimal bracket — historical body intact). No participant-flow surface changed; no email-template edit; pilot variants in DB unchanged.
>
> **D70 update (2026-06)**: line breaks in the email-template editor body now render as visible line breaks in the HTML email (not collapsed to a single space). Pure render-layer CSS addition — `white-space:pre-line` appended to every prose `<p>` wrapper in `lib/email/templates/render.ts` (lead paragraphs + all three fine-paragraph style variants; button paragraph excluded since its content is a CTA label, not prose). Plain-text rendering was already correct (no edit). Editor input unchanged. No migration, no schema change. Templates whose section bodies contain no `\n` render byte-identically to pre-D70 output.
>
> **D71 update (2026-06)**: Outlook / O365 strips the `white-space` CSS property as part of its HTML standardisation pass, so D70's CSS-only layer was insufficient for academic JUST email and any other O365 recipient. D71 adds a second defensive layer — a post-escape, post-linkify `\n → <br>\n` replacement inside the `escapedSections` construction loop in `lib/email/templates/render.ts`. Both layers ship together (Approach C / defensive layering): D70's CSS still benefits Gmail + Apple Mail; D71's explicit `<br>` tag is what Outlook honours. Single-site centralised transform, conditional on `k !== spec.buttonSection` (CTA labels stay single-line). Sequencing is load-bearing — the replace runs AFTER `escapeHtml` so the literal `<br>` survives as a tag (pre-escape it would render as visible `&lt;br&gt;` text). Plain-text path unchanged. No migration, no schema change. Default templates contain zero `\n` so their rendered output is unchanged.
>
> **D72 update (2026-06)**: `{name}` placeholder now usable in the `intro` section of participant templates (`invitation`, `reminder1`, `reminderFinal`). ALLOWED only — not required — so Sura can write `Hello {name},` OR omit it; both save and render. Brief originally proposed a 3-line allowlist edit; read-first surfaced that participant wrappers never passed `name` and `interpolate()` left unmapped tokens literal, so the proper fix expanded to also: (a) default `name` to `""` in `valuesFor()` (`render.ts`), (b) widen wrapper inputs (`SendInvitationEmailInput` + `SendReminderEmailInput`) with optional `name?: string | null`, (c) pass plaintext name from the create flow (`v.name` already in scope) and decrypt `recipient_name_encrypted` non-fatally in the resend flow + cron route. Decrypt-fail for name DEGRADES TO EMPTY (delivery is load-bearing; `Hello ,` is ugly but ships the link) — contrast with email/token/access_code where decrypt-fail aborts. PII discipline: name held only for the send, never logged, never audited. Defaults unchanged; admin-invite + submission specs untouched.
>
> **D73 update (2026-06)**: Pilot-Feedback Hub (`/admin/analytics/feedback`) now aggregates by `question_code` across all 4 active pilot variants. Each variant carries its own F1–F4 rows in `questions` (UNIQUE per `version_id`, so 16 rows total with byte-identical text per the seed audit); before D73, `getPilotFeedback()` in `lib/repos/feedback.ts` rendered 16 sections (4 per code, only 1 populated by the responder's variant), which surfaced when OFF-JOR-02 became the first real submission. Fix is pure read-aggregation in that one repo function: build `idToCode` across all variant rows, dedupe questions to one per code (lowest-`order_index` wins via ASC-iteration first-seen), re-key the in-memory answers join from `question_id` to `question_code` so any variant's answer buckets into the canonical section. Return type and page render at `feedback/page.tsx` unchanged. Cross-variant pooling is the correct unit for F1–F4 (questionnaire UX signal); a per-variant breakdown would be a follow-on main-study feature. No schema change, no migration, no DB mutation — `answers` table untouched, OFF-JOR-02's row is safe.
>
> **D74 update (2026-06)**: Owner-only export hub now lives at `/admin/exports`. Two scopes (single response × bulk) × two formats (CSV with UTF-8 BOM × XLSX via `exceljs`). Long-format: 1 row per (response × answer), 18 denormalized columns. Filters: `submitted_at IS NOT NULL` AND `status = 'active'` only (withdrawn excluded; `is_locked` not a filter). The repo (`lib/repos/exports.ts`) decrypts `recipient_name_encrypted` and `recipient_email_encrypted` inline with an ALL-OR-NOTHING posture — any decrypt failure aborts the entire export, writes a `warn`-severity audit row with `errorClass='config'` (never `err.message`), and surfaces a safe banner to the operator. One audit entry per attempt only (success → `info` with `{scope, format, responseCount, refCodes}`; failure → `warn` with `{scope, format, errorClass}`); no "started" row. Token plaintext + access-code ciphertext columns and `consent_records.signed_name_encrypted` are NEVER exported by design. Download route writes `Cache-Control: no-store, max-age=0`. **DR pointer:** a decrypt failure in production maps to the Vault key rotation / config sequence already documented below under "Disaster recovery: lost encryption key" — confirm `ENCRYPTION_KEY` env + Supabase Vault `pii_key_v1` state, then retry the export. The export is read-only across data tables; nothing was lost, nothing was mutated by the failed attempt itself.
>
> **D76 update (2026-06)**: `/admin/security` (the owner-only audit-log viewer) now ships filters, summary chips, and a 250-row page cap (was 100). Filter surface: `severity` (info / warn / alert), date range (rolling Last 24h / 7d / 30d preset, or custom `from`/`to` dates), `action` (dynamic dropdown sourced from `SELECT DISTINCT action`), `actor` (dropdown of all admins via `listAdmins`, including removed admins so historical filtering works), and `resource` (ILIKE substring). State lives in URL query params (HTML `method=GET`, no client JS) — filtered views are bookmarkable. Summary chips show **filtered** totals as drill-in feedback (when `severity=warn` is active, the other-severity chips show 0). Pagination footer reads "Showing N of M events" using Postgres `count: 'exact'` so M is the full filtered count, not the LIMIT-narrowed one. **`audit_log` is READ-ONLY across this surface** — D76 changes how reads are shaped and displayed; the `log_audit` RPC write path is unchanged.
>
> **D26 audit clarification (2026-06-05)**: pilot-window verification confirms what the live audit_log looks like. **Phase ①** (IP + UA capture via `lib/audit.ts:getRequestMeta()` over `x-forwarded-for` / `x-real-ip` / `user-agent`): WIRED — 170/170 production rows have `ip`, 163/170 have `user_agent`. **Phase ②** (`admin.login` success + `admin.login.failed` events): WIRED — 32 success rows and 24 failed-login rows present (the latter all severity `warn`, actor `system`, IP populated). **Phase ③** (country/city geo resolution): NOT WIRED — `country` / `city` columns are 0 / 170 populated, no MaxMind / `@vercel/geo` dep, no `x-vercel-ip-*` header extraction; correctly deferred. **Phase ④** (unknown-email-request failure logging): NOT WIRED — Supabase auth logs still cover this in the interim. See `docs/DECISIONS.md` D26 "Audit confirmation (2026-06-05)" for the full breakdown plus the two future paths for Phase ③.
