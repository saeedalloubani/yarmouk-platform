...
>
> **D67 update (2026-06)**: per-category labels for the 4 pilot variants. Pre-D67 `lib/i18n.ts` shipped one `categoryOfficials` + one `pilotBadge` string and rendered them for ALL categories regardless of `invitation.category`. D66 smoke (SMOKE-D66-002, category=researchers) surfaced a Researcher landing page showing "Official — Pilot Reviewer". D67 added 3 new `categoryX` keys + renamed `pilotBadge` → `pilotBadgeOfficials` with 3 new sibling badge keys, plus `categoryLabel(category, t)` + `pilotBadgeLabel(category, t)` helpers in `lib/i18n.ts`. Main-variant labels are D68 backlog. No migration, no RPC, no schema change — i18n strings + helpers + 4 consumer migrations only.
