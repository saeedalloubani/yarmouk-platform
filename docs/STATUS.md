...
| Bilingual completeness pass | Pre-launch | Five i18n strings deferred for Arabic until Sura supplies translations. (1) `ethicsFooter` — rendered English-only inline on both landing variants; pre-launch remediation: add to `lib/i18n.ts` and swap in. (2) `invalidTitle`, `invalidBody` — `/invitation-invalid` renders a literal amber-dashed Arabic placeholder; pre-launch: add keys to `lib/i18n.ts` and refactor the page to read from them. (3) `byInvitationOnly`, `contactResearcher` — keys exist in `lib/i18n.ts` with sentinel Arabic value `"[Arabic text — to be added before launch]"`; the no-session landing renders them inside amber-dashed boxes; pre-launch: replace sentinels with real Arabic and strip the amber-dashed styling. All Arabic gaps render visibly during testing — no silent English fallbacks. |
