...
**Built & live (the collection loop):** respondent flow (invite → email → consent → bilingual questionnaire → submit), admin Overview, invitations (create/send/resend/revoke), responses (list/detail with tags + researcher notes + recordings section), draft question editor, settings (notification prefs only), auth, audit logging (written), email both directions (invitation bilingual), **email-template editor wired for all 3 platform emails** (invitation bilingual, admin-invite EN-only, submission EN-only). Deployed at karasneh-research.org, 21 migrations, true-empty. See the Done checklists below.
...
- **Backups — v1 + D27 BOTH SHIPPED + restore-PROVEN.** Manual encrypted DB backup (`npm run backup` → encrypted .yarmoukbackup) restore-proven 2026-05-24 (commit 8a4c69b). **D27 automated daily backup** (GitHub Actions, 03:00 UTC, encrypted blob to Cloudflare R2) SHIPPED + RESTORE-PROVEN 2026-05-27 (merge d547c43 + fixes 4e3bb04 + f0e921a) — CI-produced blob downloaded from R2, decrypted, restored to throwaway postgres:17 with pre-seeded Supabase stubs (`authenticated` role + `auth`/`vault`/`extensions` schemas), all 17 public tables matched live counts. RUNBOOK "Backup & restore" documents the three-secrets pairing and BOTH restore procedures (manual-blob + rehearsed-CI-blob, step-exact including stubs). REMAINING CAVEATS (not blocking enrollment): (a) DB-ONLY — no Storage/audio (text-first; add when interviews recorded); (b) full project-level DR is DOCUMENTED but NOT yet rehearsed end-to-end; (c) R2 30-day lifecycle rule worth verify-after-window (~2026-06-26). The go-live readiness bar is fully met.
...
- Analytics dashboards — 1 of 5 built. ✓ Pilot-feedback hub DONE (2026-05-24, commits ebf9efb + e356148) — both-roles /admin/analytics/feedback, F1-F4 grouped, unblocks the Pilot V1→V2 wording review. REMAINING 4: per-question pivot (overlaps ATLAS.ti — low priority), themes/tags (empty until Sura tags during analysis), timeline (Overview already has funnel), demographics cross-tab (Overview has completion-by-category; this extends to category×nationality). Overview KPI tiles remain the only other dashboard.
...
- Per-template BCC owner toggle + global override (the un-built tail of D22; not blocking).
...
### Revoke invitation — owner-driven terminal kill (2026-05-31, SMOKE-PROVEN)
- [x] **`revokeInvitationAction`** (D61) — three-op terminal kill performed together: (1) rotate `token_hash` to a freshly-minted hash whose plaintext is discarded (old link permanently dead — same primitive resend uses); (2) set `status='revoked'` (terminal label for the admin UI); (3) `is_locked=TRUE` on every non-submitted response (kicks active session via `getSession` / blocks `saveAnswer`; saved answers RETAINED, not destroyed — owner still reads them). Block-then-confirm gate: action returns `error:"in_progress"` unless `confirmHardRevoke=true`; UI surfaces the honest second confirmation ("their saved answers are retained and visible to you, but they cannot add more or submit"). Submitted-response block unconditional (mirrors resend's `already_submitted`). Race closure: the POST-rotation re-read is the canonical lock target — validate_invitation_token is the sole creator of `responses` rows and its `SELECT…FOR UPDATE` serialises against the rotation UPDATE, so any sneak-in commits before rotation (visible) or sees the new hash (no insert). Audit `invitation.revoke` (severity=warn), metadata is `hadInProgressResponse` + `lockedResponseIds` only (no PII).
- [x] **Migration `20260527130001_invitation_status_revoked.sql`** — adds `'revoked'` to the `invitation_status` enum via `ALTER TYPE … ADD VALUE IF NOT EXISTS` (PG12+ tx-safe; no same-tx usage). Commits before code that writes the value goes live (deploy-order coordinated by Saeed).
- [x] **UI** — `components/InvitationRevokeButton.tsx` (window.confirm two-step matching resend's pattern; `router.refresh()` on `already_revoked` self-corrects stale tabs without instructing the user). Page `app/admin/(protected)/invitations/page.tsx`: hide BOTH Resend and Revoke when `status==='submitted' || status==='revoked'` (terminal); revoked chip uses `bg-dangerLight text-danger` (matches `tailwind.config.ts` danger tokens; submitted stays brand — done is success-terminal, killed is danger).
- [x] **Prod smoke (2026-05-31) — Cases 1/2/4 PASS** against `main_researchers` v1 (throwaway activate; reverted to draft post-smoke, byte-identical pre/post per the 4-count residue gate — invs/resps/answs/qns all zero). Case 1 (no-response revoke): link dies, chip flips danger, both buttons vanish from the row. Case 2 (in-progress): respondent click via `/r/<token>` creates the response row (validate runs even on 0-question version), revoke second-confirm fires, `✓ Revoked · session locked`, audit shows `lockedResponseIds`, re-click → `/invitation-invalid`. Case 4 (already_revoked staleness): two-tab probe, `router.refresh()` re-renders the row terminal and unmounts the controls. Case 3 skipped (already_submitted path is identical to resend's, already exercised). Commits: `baef0f3` (migration + action + types), `ca48e96` (UI), `6981fd7` (types regen post-`supabase db push`). Decision: **D61**. **Retires two standing items:** Activate/Close UI button final confirmation (exercised on `main_researchers` and the accidental `pilot_officials` activate during smoke — button correctly flips status + row updates), and Task #11 token-rotation docs (covered by the action headers + RUNBOOK "Revoking an invitation").

### Email-template editor — Stage 2 done (2026-05-31, SMOKE-PROVEN, DECISION D62)
- [x] **All 3 platform-sent emails are editor-managed**: `invitation` (bilingual, Stage 1), `admin-invite` (EN-only supervisor magic-link, Stage 2), `submission` (EN-only owner-side "a response was submitted" notification, Stage 2). One registry-driven flow (TEMPLATE_SPECS in `lib/email/templates/types.ts`), one renderer, one editor surface.
- [x] **Migration `20260531120001_email_templates_id_widen.sql`** — widens `email_templates.id` CHECK enum (drop+re-add in a single tx) to allow `'admin-invite'` and `'submission'` alongside `'invitation'` (and the 4 pre-existing aspirational ids, kept). Tx-safe, idempotent, no data migration.
- [x] **Renderer generalized** — `renderInvitationEmail` → `renderEmailTemplate` (clean rename, no permanent alias). Layout descriptor in `TemplateSpec` (`buttonSection` + `placement` + `linkify`) replaces the hard-wired invitation layout. Divider OMITTED when no fine sections (submission). **Invitation output remains byte-equivalent** to pre-Stage-2 — verified per-paragraph against the old hardcoded HTML + plain text. The 5-section invitation shape (intro / cta / personal / expiry / contact) is one composition of the new general layout, not a special case.
- [x] **Chrome unification — intentional, accepted (D62)**: admin-invite gains +1px font on greeting/intro (15→16) and -2px on the greeting margin (18→16); brand-consistent with invitation, below real-world email-client variance. Submission migrates from a bare div to the same branded card + blue button used by the other two templates — owner-only recipient, strict visual improvement, structural button guarantee.
- [x] **EN-only template support** — `TemplateSpec.bilingual=false` cascades through the editor (AR column HIDDEN, not empty fields) and the action layer (per-locale validation skipped for missing AR). Schema already supported `subject_ar` / `sections_ar` nullable from Stage 1's `20260527120001` migration; no further DB change.
- [x] **Thin wrappers** — `lib/email/admin-invite.ts` and `lib/email/submission.ts` rewritten to mirror the post-D22 `invitation.ts` pattern (load overlay via service-role → `resolveTemplate` → `renderEmailTemplate` → Resend). Exported function signatures **unchanged**; callers in `lib/actions/admins.ts:279` and `lib/notifications.ts:97` not touched. Submission's `href` stays optional to match `notifications.ts`'s `emailHref` derivation; when undefined the wrapper aborts `{ok:false}` and the in-app notification (which has a working relative href) still lands — owner-side is degraded gracefully, never with a broken-button email.
- [x] **Editor + list page** — `EmailTemplateEditor.tsx` gains 3 new `SECTION_LABELS` entries (greeting / notice / lead) + softened cta hint + softened placeholder-reference paragraph (less invitation-specific). List page widens `TEMPLATES = ["invitation", "admin-invite", "submission"]`. Per-template inert URL `${SITE_URL}/?preview=${id}-email` in both `previewTemplateAction` and `sendTestEmailAction`; public landing ignores all query strings (verified in `app/(public)/page.tsx` — no `searchParams` prop).
- [x] **Prod smoke (2026-05-31)** — Saeed test-sent all three templates to his inbox: branded card chrome on all three (unification proof), placeholder substitution correct (`Hello Dr. Example,` for admin-invite, `(EX-001)` for submission), inert URLs land on the public landing page with no token consumed. EN-only UI confirmed (admin-invite + submission show NO Arabic column; invitation shows both as expected). Edit→test→reset cycle on admin-invite exercised — `template.reset` audit row written + chip flipped back to `default`. Leftover D22 Stage 1 customization on Participant invitation cleared via UI Reset (same `resetTemplateAction` path). Commits: `28d0951` (migration), `8a96e89` (code), PR merged 2026-05-31 as `55002f2`. Decision: **D62**.
...
1. **BACKUPS — ✓ v1 + D27 BOTH SHIPPED + restore-PROVEN.** Manual encrypted DB backup (`npm run backup` → encrypted `.yarmoukbackup`) round-trip proven 2026-05-24 (commit 8a4c69b). **D27 automated daily backup** (GitHub Actions, 03:00 UTC → Cloudflare R2, encrypted, Vault-blind via backup_ro role) SHIPPED + RESTORE-PROVEN 2026-05-27 (merge d547c43 + fixes 4e3bb04, f0e921a) — CI-produced blob downloaded from R2, decrypted, restored to throwaway `postgres:17` with the required Supabase stubs (`authenticated` role + `auth`/`vault`/`extensions` schemas), all 17 public tables match live counts. RUNBOOK "Backup & restore" documents both restore procedures step-exact (manual-blob + rehearsed-CI-blob including the stubs vanilla pg needs). Go-live readiness fully met. Follow-on caveats (NOT blocking): DB-only (no Storage/audio — text-first), full project-level DR documented-but-not-rehearsed, R2 30-day lifecycle verify-after-window. See "## What's Left" Tier 1.
