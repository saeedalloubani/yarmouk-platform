...
- Geo/IP/device capture (D26) — STILL OPEN, scoped 2026-05-24 (read-only investigation; privacy-cleared). Findings: (1) PRIVACY-SAFE — D26 is strictly ADMIN-action logging; confirmed no respondent IP is captured anywhere (token route + respondent flow write no audit rows / read no headers); does NOT touch the D4 anonymity promise. (2) Capture must be APP-LAYER, not the trigger — Postgres can't see the request; lib/audit.ts must read headers() (x-forwarded-for, user-agent) and the log_audit RPC must grow p_ip/p_user_agent/p_country/p_city params (trigger won't overwrite app-supplied values). (3) BIGGEST HIDDEN SCOPE: D26's headline use-case (login from a new country) and D25's failed-login logging are NOT served by enriching existing rows — the auth/login path writes NO audit row today. Delivering D26's intent requires a NEW login-audit event at the magic-link callback, not just IP on mutation rows. (4) Geo RESOLUTION (MaxMind GeoLite2 → country/city) is a separate, heavier piece from raw IP/UA capture — capture-now / resolve-later is viable since the columns already exist. NOT a quick build; needs the login-event design + the MaxMind dependency decision. Good fresh-session opener.
