...
| **Resend sending domain (LOAD-BEARING — blocks real sends)** | Pre-launch | Invitation email is live (3b-ii) but the `from` is `onboarding@resend.dev`, whose test sender **only delivers to the Resend account address** (`salloubani@cybercorrelate.com`) — confirmed live in 3b-ii smoke (a send to a different address was correctly rejected). Before launch: (1) verify a real domain at resend.com/domains (SPF/DKIM/DMARC on `karasneh-research.org`), and (2) change the `from` constant in **both** `lib/email/invitation.ts` **and** `lib/email/submission.ts` off `onboarding@resend.dev` (the response-submitted notification email re-declares the same `FROM`/`REPLY_TO` — both copies must change together; see the shared-extract item below). **Paired pre-launch item:** Sura provides Arabic invitation-email copy — `AR` in `lib/email/invitation.ts` currently falls back to `EN`; both this and the domain gate must be done before real (especially Arabic) officials are emailed. |
...
| Notification preferences — honor toggles + UI | Session 5 | `notification_preferences` (per-admin × per-event × per-channel, seeded defaults TRUE) is **not honored** by the response-submitted fan-out — it's default-on (no rows seeded → reading is a no-op today). Honoring it later is a trivial `admins LEFT JOIN notification_preferences` with `COALESCE(np.submission_*, true)` once a preferences UI exists to set the toggles. Build the UI + wire honoring in Session 5 (Comms). |
