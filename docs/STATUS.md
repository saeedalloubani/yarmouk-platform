...
| **2nd researcher account — `salloubani@cybercorrelate.com` (permanent, ethics-gated)** | Decided 2026-05-23 | NO LONGER a removal blocker. Saeed stays as a permanent second `owner` — collaborating on the thesis, collection, and analysis. **CONDITION (load-bearing):** this means TWO researchers have access to confidential participant PII, so Sura's ethics approval AND the consent-form language ("accessible only to the researcher") MUST be updated to reflect two named researchers with data access before real participants are enrolled. The database fact (2nd owner) must not get ahead of the paperwork. Provisioned in prod per the Production Deployment checklist (steps 2–3). |
| **Resend sending domain — from-address swap remaining (LOAD-BEARING)** | Pre-launch | Domain `karasneh-research.org` **VERIFIED 2026-05-23** (Cloudflare DNS — DKIM `resend._domainkey`, SPF/MX on the `send` subdomain, DMARC `p=none`); no longer pending verification. **REMAINING:** swap the `from` constant off `onboarding@resend.dev` in **both** `lib/email/invitation.ts` **and** `lib/email/submission.ts` (both re-declare the same `FROM`/`REPLY_TO` — change together; see the shared-extract item below) to a `karasneh-research.org` sender — Production Deployment checklist step 6. **Paired pre-launch item:** Sura provides Arabic invitation-email copy — `AR` in `lib/email/invitation.ts` currently falls back to `EN`; do before real (especially Arabic) officials are emailed. |
...
**Status as of 2026-05-23: NO PRODUCTION EXISTS YET.** Nothing deployed, no real data anywhere. The steps below are the remaining runway.
...
### Remaining deploy checklist (next session, fresh head — do in order)

1. **Vercel** — connect repo `saeedalloubani/yarmouk-platform`, set env vars. **CRITICAL:** the Supabase URL / anon / service-role keys MUST be `yarmouk-platform`'s (ref `trvxugvkesfcopwdtdey`) — triple-check the ref. Plus `RESEND_API_KEY`, `NEXT_PUBLIC_SITE_URL=https://karasneh-research.org`. `ENCRYPTION_KEY` lives in Vault (already set), NOT in Vercel env.
2. **Auth bootstrap** (per RUNBOOK) — disable signups; pre-create auth identities for Sura (`sjkarasneh24@eng.just.edu.jo`) + Saeed (`salloubani@cybercorrelate.com`, permanent 2nd researcher — ethics-cleared); set redirect URLs incl. `https://karasneh-research.org/admin/callback`.
3. **Seed `admins` rows (owner)** to match the auth identities — confirm via migration or manual.
4. **Point `karasneh-research.org` at Vercel** (Cloudflare DNS — the SITE domain, separate from the `send` subdomain Resend uses).
5. **Backups ON** before any real response (D27/D28; `BACKUP_PASSPHRASE` to the password manager).
6. **From-address swap** — `lib/email/invitation.ts:19` + `lib/email/submission.ts:25`, `onboarding@resend.dev` → a `karasneh-research.org` sender (confirm the exact allowed from-address in Resend).
7. **Production smoke** — create invitation → real email delivers → respondent opens link → fills → submits → owner sees it. Then true-empty cleanup of the smoke.
