...
**Audit confirmation (2026-06-05):** read-only verification against live production data with 170 audit_log rows and the pilot active.

- **Phase ①** — IP + UA capture: **WIRED** end-to-end. The capture point is `lib/audit.ts:getRequestMeta()` (best-effort read of `headers()` from `next/headers`, request-scoped). IP = first hop of `x-forwarded-for`, falling back to `x-real-ip`. UA = `user-agent` header. Auto-invoked from `logAudit` (authenticated path; flows through `log_audit` RPC params `p_ip` + `p_user_agent`), and from the three service-role direct-INSERT wrappers `logSystemEmailFailure`, `logFailedLogin`, and `logFailedAccessCode` (each calls `getRequestMeta` and inserts `ip` + `user_agent` straight onto the row). Production: `audit_log.ip IS NOT NULL` for 170/170 rows; `audit_log.user_agent IS NOT NULL` for 163/170 rows (the 7 missing UA rows are degraded captures, not bypassed paths). All four wrappers verified.
- **Phase ②** — `admin.login` success + `admin.login.failed` events: **WIRED** and exercised. `app/admin/callback/route.ts` writes `admin.login` (authenticated) on the success path and calls `logFailedLogin` on the verify-failure / missing-params paths (service-role direct insert, actor=`system`, narrow reason union `verify_failed | missing_params`). Production: 32 `admin.login` success rows + 24 `admin.login.failed` rows, the latter all severity=`warn` with IP populated (e.g. a 2026-06-01 cluster from a retry session — 21 failures across ~35 minutes from a single IP, exactly the operational pattern Phase ② was designed to surface).
- **Phase ③** — country/city geo resolution: **NOT WIRED**, correctly deferred. `package.json` has no geo-lookup dep (no MaxMind, `@vercel/geo`, `ip2location`, etc.); no `x-vercel-ip-country` / `x-vercel-ip-city` header extraction anywhere in `lib/` or `middleware.ts`. Production: `country IS NOT NULL` = 0 rows; `city IS NOT NULL` = 0 rows. Two future paths if/when this is picked up: (a) the originally-planned MaxMind GeoLite2 + license key, resolve-on-read; or (b) the lighter-touch alternative of reading Vercel's auto-injected `x-vercel-ip-country` / `x-vercel-ip-city` request headers inside `getRequestMeta` and extending `log_audit`'s signature with `p_country` + `p_city`. (b) is a single migration + ~6-line patch and pays no per-request lookup cost; (a) gives city-level precision and is portable off Vercel.
- **Phase ④** — unknown-email-request failure logging: **NOT WIRED**, status unchanged. Saeed's Supabase auth logs still cover this in the interim.

**Out of audit scope:** no code touched; this is a documentation amendment only. The `audit_log` schema, the `log_audit` RPC, and the `lib/audit.ts` write paths are all unchanged.
